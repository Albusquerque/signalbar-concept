...
To refresh event or weather frames after changing SignalBar's renderer, check out the SignalBar repository as a sibling directory and run `python3 export_frames.py` or `python3 export_weather_frames.py`. The Playwright smoke test is `node test_site.mjs` when Playwright is available in the parent workspace.
